function [bg_mask fg_mask] = classify_using_kde_sharpening_sigma_Sheikh_normalization( img_pixels, bg_model, bg_indicator, bg_sigmas, bg_prior, bg_near_rows, bg_near_cols, fg_model, fg_indicator, fg_sigmas, fg_prior, fg_near_rows, fg_near_cols, num_feature_vals, debug_flag)
%function [bg_mask fg_mask] = classify_using_kde_sharpening_sigma_Sheikh_normalization( img_pixels, bg_model, bg_indicator, bg_sigmas, bg_prior, bg_near_rows, bg_near_cols, fg_model, fg_indicator, fg_sigmas, fg_prior, fg_near_rows, fg_near_cols, num_feature_vals, debug_flag)
...
%Uniform distribution added to the fg model is 1/256/256/256 although Sheikh and Shah use 1/256/256/256/num_rows/num_cols -- since we use a pixel-wise prior and classification
...
for res=1:num_resolutions
    ...
    [bg_liks{res} bg_sigmas_image{res} bg_model_sigmas{res}] = calculate_kde_likelihood_sharpening_Sheikh_normalization( img_pixels{res}, bg_model_sampled, bg_indicator_sampled, bg_sigma_XYs{res}, bg_sigma_Ys{res}, bg_sigma_UVs{res}, bg_near_rows, bg_near_cols, 0, num_feature_vals, debug_flag);
    %Calculate KDE likelihoods for fg
    [fg_liks{res} fg_sigmas_image{res} fg_model_sigmas{res}] = calculate_kde_likelihood_sharpening_Sheikh_normalization( img_pixels{res}, fg_model{res}, fg_indicator{res}, fg_sigma_XYs{res}, fg_sigma_Ys{res}, fg_sigma_UVs{res}, fg_near_rows, fg_near_cols, 0, num_feature_vals, debug_flag);
end
...
new_liks_prior = (1/256/256/256).*new_prior_3;
%Compute masks
